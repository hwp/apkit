...
def _complex_coherence(x, y, win_size, fs):
    freq_bins, csd = scipy.signal.csd(x, y, nperseg=win_size, fs=fs)
    coherence = csd / np.sqrt(scipy.signal.welch(x, nperseg=win_size, fs=fs)[1].real
                              * scipy.signal.welch(y, nperseg=win_size, fs=fs)[1].real)
    return freq_bins, coherence
...
def main(infile, outfile, win_size, plot_title, plot_max_freq):
    ...
    for i in range(n_ch - 1):
        for j in range(i + 1, n_ch):
            freq_bins, coherence = _complex_coherence(sig[i], sig[j], 
                                                      win_size=win_size, fs=fs)
            with open(f'{outfile}_{i}vs{j}', 'w') as f:
                for freq, x in zip(freq_bins, coherence):
                    print(f'{freq}\t{x.real}\t{x.imag}', file=f)

    with open(f'{outfile}_plot.gp', 'w') as f:
        print('set terminal pngcairo size 800,600', file=f)
        print(f'set xrange [0:{plot_max_freq}]', file=f)
        print('set xlabel "Frequency"', file=f)
        print('set yrange [-.5:1]', file=f)
        ...
        for i in range(n_ch - 1):
            for j in range(i + 1, n_ch):
                ...
                print(f"plot '{outfile}_{i}vs{j}' u 1:2 smooth csplines w l lc 'green' title 'Real', "
                            "'' u 1:3 smooth csplines w l lc 'red' title 'Imaginary', "
                            "'' u 1:(sqrt($2**2+$3**2)) smooth csplines w l lc 'blue' dt '.' title 'Magnitude'",
                      file=f)
